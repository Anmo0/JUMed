...
import { 
  ...
  getBatches, getLocationRestrictionSetting, setLocationRestrictionSetting,
...
function getDistance(loc1: { latitude: number; longitude: number }, loc2: { latitude: number; longitude: number }): number {
    const R = 6371e3; 
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [state, setState] = useState<AppState>({
    batches: [], students: [], groups: [], lectures: [], attendance: [], courses: [],
    absenceWeight: 2.5, deviceBindingEnabled: true, absencePercentageEnabled: true, locationRestrictionEnabled: false,
  ...
  const { data: globalSettings } = useQuery({
      ...
      queryFn: async () => {
          const [bindingEnabled, percentageEnabled, weightSetting, locationEnabled] = await Promise.all([
              getDeviceBindingSetting(), getAbsencePercentageSetting(), getAbsenceWeightSetting(),
              getLocationRestrictionSetting().catch(() => false)
          ]);
          return { bindingEnabled, percentageEnabled, weightSetting, locationEnabled };
      }
  });
  ...
  const { data: batchData } = useQuery({
      ...
      queryFn: async () => {
          const [studentList, groupList, { data: batchInfo }] = await Promise.all([
              getStudents(state.selectedBatchId!), getGroups(state.selectedBatchId!),
              supabase.from('batches').select('current_year').eq('id', state.selectedBatchId!).single()
          ]);
          
          let query = supabase.from('courses').select('*');
          if (batchInfo?.current_year) query = query.eq('academic_year', batchInfo.current_year);
          ...
          const formattedCourses = (coursesData || []).map((c: any) => ({
              ...
              creditHours: c.credit_hours, weeks: c.weeks, absenceLimit: c.absence_limit || 25,
              absenceWeight: c.absence_weight ?? 2.5, createdAt: c.created_at
          }));

          return { students: studentList, groups: groupList, courses: formattedCourses };
      }
  });
  ...
  useEffect(() => {
      if (globalSettings || fetchedBatches || fetchedLectures || fetchedAttendance) {
          updateState(prev => ({
              ...prev,
              isLoading: false,
              deviceBindingEnabled: globalSettings?.bindingEnabled ?? prev.deviceBindingEnabled,
              absencePercentageEnabled: globalSettings?.percentageEnabled ?? prev.absencePercentageEnabled,
              absenceWeight: globalSettings?.weightSetting ?? prev.absenceWeight,
              locationRestrictionEnabled: globalSettings?.locationEnabled ?? prev.locationRestrictionEnabled,
              batches: fetchedBatches ?? prev.batches,
              lectures: fetchedLectures ?? prev.lectures,
              attendance: fetchedAttendance ?? prev.attendance,
          }));
      }
  }, [globalSettings, fetchedBatches, fetchedLectures, fetchedAttendance, updateState]);

  useEffect(() => {
      if (batchData) {
          updateState({ students: batchData.students, groups: batchData.groups, courses: batchData.courses });
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('attendance-changes')
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'attendance' }, (payload) => {
            const newRecord = {
                ...
                timestamp: payload.new.timestamp, location: payload.new.location, lectureId: payload.new.lecture_id,
                isOutsideRadius: payload.new.is_outside_radius, manualEntry: payload.new.manual_entry, distance: payload.new.distance,
            };
            pendingAttendance.current.push(newRecord);
        })
        .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'attendance' }, (payload) => {
            updateState(prev => ({ ...prev, attendance: prev.attendance.filter(a => a.id !== payload.old.id) }));
        }).subscribe();
    ...
    const flushInterval = setInterval(() => {
        if (pendingAttendance.current.length > 0) {
            ...
            updateState(prev => {
                const existingIds = new Set((prev.attendance || []).map(a => a.id));
                const uniqueNew = batch.filter(record => !existingIds.has(record.id));
                if (uniqueNew.length === 0) return prev;
                return { ...prev, attendance: [...(prev.attendance || []), ...uniqueNew] };
            });
        }
    }, 3000);
    
    return () => { supabase.removeChannel(channel); clearInterval(flushInterval); };
  ...
  const filteredLectures = useMemo(() => {
      return (state.lectures || []).filter(l => l.batchId === state.selectedBatchId);
  }, [state.lectures, state.selectedBatchId]);

  const filteredAttendance = useMemo(() => {
      const lectureIds = new Set(filteredLectures.map(l => l.id || l.qrCode));
      return (state.attendance || []).filter(a => lectureIds.has(a.lectureId));
  ...
  const selectBatch = useCallback((batchId: string | null) => {
    ...
    if (batchId) localStorage.setItem('selectedBatchId', batchId);
    else localStorage.removeItem('selectedBatchId');
  }, [updateState]);

  const refreshStudents = useCallback(async () => {
      await queryClient.invalidateQueries({ queryKey: ['batchData', state.selectedBatchId] });
  }, [queryClient, state.selectedBatchId]);
  ...
  const updateStudent = useCallback(async (id: string, updates: Partial<Student>) => {
    ...
    if (error) { toast.error(error); return; }
    if (updatedStudent) {
      updateState(prev => ({ students: (prev.students || []).map(s => s.id === id ? { ...s, ...updatedStudent } : s) }));
  ...
  const deleteGroupLocal = useCallback(async (groupId: string) => {
    toast((t) => (
      <div className="flex flex-col gap-3 text-right" style={{ direction: 'rtl' }}>
        <div>
          {/* 💡 تم تعديل الألوان لتناسب الخلفية الداكنة وثيم رمضان */}
          <p className="font-bold text-white text-base">هل أنت متأكد من حذف هذه المجموعة؟</p>
          <p className="text-xs text-gray-400 mt-1">سيتم إخراج جميع الطلاب منها، ولا يمكن التراجع.</p>
        </div>
        <div className="flex gap-2 mt-2">
          <button 
            ...
            className="flex-1 bg-red-600 hover:bg-red-700 text-white px-3 py-2 rounded-xl text-sm font-bold transition-all shadow-lg"
          >
            تأكيد الحذف
          </button>
          <button 
            onClick={() => toast.dismiss(t.id)} 
            className="flex-1 bg-slate-700 hover:bg-slate-600 text-white border border-slate-600 px-3 py-2 rounded-xl text-sm font-bold transition-all"
          >
            إلغاء
          </button>
        </div>
  ...
  const deleteAllGroupsLocal = useCallback(() => {
    updateState(prev => ({
        groups: [], students: (prev.students || []).map(s => ({ ...s, groupId: undefined, groupName: undefined, isLeader: false }))
    }));
  }, [updateState]);

  const generateQrCode = useCallback((details: { date: string, timeSlot: string, courseName: string, courseId?: string, batchId: string, isManual?: boolean }, callbacks: { onSuccess: () => void, onError: (message: string) => void }) => {
    let activeUserId = user?.id; let activeUserRole = user?.role;
    const localUserStr = localStorage.getItem('anmo_current_user');
    if (localUserStr) { try { const localUser = JSON.parse(localUserStr); activeUserId = localUser.id; activeUserRole = localUser.role; } catch (e) {} }

    const student = state.students.find(s => s.id === activeUserId);
    const isLeader = student?.isBatchLeader || student?.canManageAttendance;

    if (activeUserRole !== UserRole.Admin && !isLeader) { callbacks.onError('ليس لديك صلاحية إنشاء محاضرة.'); return; }
    if (!details.batchId) { callbacks.onError('يرجى اختيار الدفعة أولاً.'); return; }
    ...
    const createLecture = async (location: { latitude: number; longitude: number }) => {
        const newLectureData: Omit<Lecture, 'id'> = {
            ...
            courseName: details.courseName, courseId: details.courseId, batchId: details.batchId, date: details.date,
            timeSlot: details.timeSlot, createdAt: new Date().toISOString(), location: location,
        };
        ...
        } else { callbacks.onError("فشل حفظ المحاضرة في قاعدة البيانات."); }
    };
    ...
    if (details.isManual) {
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => createLecture({ latitude: position.coords.latitude, longitude: position.coords.longitude }),
  ...
  const clearLectureAttendance = useCallback(async (lectureIdentifier: string) => {
      ...
      if (error) toast.error(error);
          ...
          updateState(prev => ({ attendance: (prev.attendance || []).filter(a => a.lectureId !== actualId && a.lectureId !== actualQr) }));
  ...
  const manualAttendance = useCallback(async (studentId: string, lectureId: string) => {
      ...
      const newRecordData: Omit<AttendanceRecord, 'id'> = {
          studentId, studentName: student.name, timestamp: new Date().toISOString(), location: lecture.location,
          lectureId: lecture.id || lectureId, isOutsideRadius: false, manualEntry: true, distance: 0,
      };

      if (!navigator.onLine) {
          const offlineQueue = JSON.parse(localStorage.getItem('anmo_offline_attendance') || '[]');
          offlineQueue.push(newRecordData); localStorage.setItem('anmo_offline_attendance', JSON.stringify(offlineQueue));
          updateState(prev => ({ attendance: [...(prev.attendance || []), { ...newRecordData, id: `temp-${Date.now()}` } as AttendanceRecord] }));
          toast.success(`تم تحضير ${student.name} محلياً`);
          return;
      }
  ...
  const removeAttendance = useCallback(async (studentId: string, lectureId: string) => {
    ...
    if (!error) updateState(prev => ({ attendance: (prev.attendance || []).filter(rec => !(rec.studentId === studentId && rec.lectureId === lectureId)) }));
  }, [updateState]);
  ...
  const recordAttendance = useCallback(async (location: { latitude: number; longitude: number }): Promise<{ success: boolean, message: string }> => {
      ...
      const localUserStr = localStorage.getItem('anmo_current_user');
      if (localUserStr) try { activeUserId = JSON.parse(localUserStr).id; } catch (e) {}
      ...
      const newRecordData: Omit<AttendanceRecord, 'id'> = {
          studentId: activeUserId, studentName: student.name, timestamp: new Date().toISOString(), location: location,
          lectureId: lectureIdentifier, isOutsideRadius: isOutsideRadius, distance: Math.round(distance),
      };
      ...
      if (!navigator.onLine) {
          ...
          offlineQueue.push(newRecordData); localStorage.setItem('anmo_offline_attendance', JSON.stringify(offlineQueue));
          updateState(prev => ({ attendance: [...prev.attendance, { ...newRecordData, id: `temp-${Date.now()}` } as AttendanceRecord] }));
          return { success: true, message: "تم تسجيل حضورك محلياً لعدم وجود اتصال." };
      }
  ...
  const repeatPreviousAttendance = useCallback(async (targetLectureId: string) => {
      try {
          ...
          const previousLectures = state.lectures.filter(l => l.batchId === currentLecture.batchId && (l.courseId === currentLecture.courseId || (l.courseName === currentLecture.courseName)) && l.id !== currentLecture.id && new Date(l.createdAt) < new Date(currentLecture.createdAt)).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
          ...
          if (prevAttendance.length === 0) return { success: false, message: 'المحاضرة السابقة لا تحتوي على حضور لنسخه.' };

          const existingIds = new Set(state.attendance.filter(a => a.lectureId === currentLecture.id || a.lectureId === currentLecture.qrCode).map(a => a.studentId));
          const newRecords = prevAttendance.filter(a => !existingIds.has(a.studentId)).map(a => ({
              studentId: a.studentId, studentName: a.studentName, timestamp: new Date().toISOString(), location: currentLecture.location,
              lectureId: currentLecture.id || currentLecture.qrCode, isOutsideRadius: false, manualEntry: true, distance: 0
          }));

          if (newRecords.length === 0) return { success: true, message: 'جميع الطلاب مسجلون بالفعل هنا.' };
          const result = await apiAddBulkAttendanceRecords(newRecords);
          if (result.error) return { success: false, message: result.error };

          if (result.data) updateState(prev => ({ attendance: [...(prev.attendance || []), ...result.data] }));
  ...
  const resetStudentDevice = useCallback(async (studentId: string) => {
    ...
    if (!error && updatedStudent) {
        updateState(prev => ({ students: (prev.students || []).map(s => s.id === studentId ? updatedStudent : s) }));
  ...
  const resetAllDevices = useCallback(async () => {
    ...
    if (!error) {
        updateState(prev => ({ students: (prev.students || []).map(s => ({ ...s, deviceInfo: undefined })) }));
        toast.success('تم إلغاء ربط جميع الأجهزة بنجاح.');
    }
  }, [updateState]);

  const toggleDeviceBinding = useCallback(async (enabled: boolean) => {
    await apiSetDeviceBindingSetting(enabled); updateState({ deviceBindingEnabled: enabled }); toast.success(enabled ? 'تم تفعيل ربط الأجهزة' : 'تم تعطيل ربط الأجهزة');
  }, [updateState]);

  const toggleAbsencePercentage = useCallback(async (enabled: boolean) => {
    await apiSetAbsencePercentageSetting(enabled); updateState({ absencePercentageEnabled: enabled }); toast.success(enabled ? 'تم تفعيل نسبة الغياب' : 'تم تعطيل نسبة الغياب');
  }, [updateState]);

  const updateAbsenceWeight = useCallback(async (weight: number) => {
      await apiSetAbsenceWeightSetting(weight); updateState({ absenceWeight: weight });
  }, [updateState]);
  ...
  const toggleLocationRestriction = useCallback(async (enabled: boolean) => {
    await setLocationRestrictionSetting(enabled); updateState({ locationRestrictionEnabled: enabled }); toast.success(enabled ? 'تم تفعيل قيود الموقع' : 'تم تعطيل قيود الموقع');
  }, [updateState]);

  const setBatches = useCallback((action: React.SetStateAction<Batch[]>) => { updateState(prev => ({ batches: typeof action === 'function' ? action(prev.batches || []) : action })); }, [updateState]);
  const setCourses = useCallback((action: React.SetStateAction<Course[]>) => { updateState(prev => ({ courses: typeof action === 'function' ? action(prev.courses || []) : action })); }, [updateState]);
